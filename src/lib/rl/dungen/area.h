...
namespace std {
	template <> struct less<std::weak_ptr<pgn::rl::cArea>>
	{
		bool operator()(const std::weak_ptr<pgn::rl::cArea> & lhs, const std::weak_ptr<pgn::rl::cArea> & rhs) const
		{
			auto p0 = lhs.lock();
			auto p1 = rhs.lock();
...
namespace std {
	template <> struct less<pgn::rl::cArea>
	{
		bool operator()(const pgn::rl::cArea & lhs, const pgn::rl::cArea & rhs) const
		{
...
inline bool operator < (const std::shared_ptr<pgn::rl::cArea> & lhs, const std::shared_ptr<pgn::rl::cArea> & rhs)
{
	return lhs->mId < rhs->mId;
}