...
namespace std {
	template <> struct less<std::weak_ptr<pgn::rlut::cArea>>
	{
		bool operator()(const std::weak_ptr<pgn::rlut::cArea> & lhs, const std::weak_ptr<pgn::rlut::cArea> & rhs) const
		{
			auto p0 = lhs.lock();
			auto p1 = rhs.lock();
...
namespace std {
	template <> struct less<pgn::rlut::cArea>
	{
		bool operator()(const pgn::rlut::cArea & lhs, const pgn::rlut::cArea & rhs) const
		{
...
inline bool operator < (const std::shared_ptr<pgn::rlut::cArea> & lhs, const std::shared_ptr<pgn::rlut::cArea> & rhs)
{
	return lhs->mId < rhs->mId;
}